...
- Replace the generic project description with a real description of what kit_hub does.
...
If **no**, remove those from `src/kit_hub/config/webapp/` and the webapp routers/middleware.
...
Replace the sample environment variable names with the real ones needed by kit_hub.
Remove any variables that are no longer relevant.

## Update `.github/agents/`

Review each agent definition file. Either:
- Update the agent description and commands to reflect kit_hub, or
...
that are not relevant to kit_hub. Keep only what will be actively used.

## Update `README.md`

Replace the template boilerplate with a real description of kit_hub:
