...
1. Configure the following:
   - **Name**: Your app name (e.g., "Kit Hub Dev")
...
Create a `.env` file at `~/cred/kit_hub/.env`:
...
uv run uvicorn kit_hub.webapp.app:app --reload

# Or using the app module
uv run python -m kit_hub.webapp.app
```
...
using the [cdn_load.sh](https://github.com/Pitrified/kit-hub/blob/main/scripts/webapp/cdn_load.sh) script.
...
3. Configure:
   ...
   - **Start Command**: `uvicorn kit_hub.webapp.app:app --host 0.0.0.0 --port $PORT`
...
1. Create a new router in `src/kit_hub/webapp/routers/`
2. Include it in `main.py`:
   ```python
   from kit_hub.webapp.routers import new_router
