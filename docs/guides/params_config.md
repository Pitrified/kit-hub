...
|---|---|---|
| **Config** | `src/kit_hub/config/` | Defines the *shape* of settings as Pydantic models |
| **Params** | `src/kit_hub/params/` | Loads *actual values* and produces a Config via `to_config()` |
...
- [`SampleConfig`](../../reference/kit_hub/config/sample_config/) - config side
- [`SampleParams`](../../reference/kit_hub/params/sample_params/) - params side
...
1. **Single constructor argument**: `env_type: EnvType | None = None`.
   Pass it explicitly from the caller (e.g., `KitHubParams`).  When
...
from kit_hub.params.env_type import (
    EnvLocationType, EnvStageType, EnvType,
    UnknownEnvLocationError, UnknownEnvStageError,
)
from kit_hub.config.sample_config import NestedModel, SampleConfig
...
`KitHubParams` (the top-level singleton) owns all Params instances
...
```python
from kit_hub.params.env_type import EnvLocationType, EnvStageType, EnvType
