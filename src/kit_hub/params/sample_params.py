...
``src/kit_hub/config/sample_config.py``.

Config / Params split:

* **Config** (``src/kit_hub/config/``) defines only the *shape* of
...
See Also:
    ``SampleConfig`` - the paired config model in ``src/kit_hub/config/``.
...
from kit_hub.config.sample_config import NestedModel
from kit_hub.config.sample_config import SampleConfig
