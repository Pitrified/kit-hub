...
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/kit_hub/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/kit_hub/` – Main application code
  - `src/kit_hub/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/kit_hub/.env`.
- Secrets should not be committed.

## Related

- `src/kit_hub/config/kit_hub_paths.py`
